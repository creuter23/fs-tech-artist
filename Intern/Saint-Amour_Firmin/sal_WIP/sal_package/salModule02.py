...
class Image_field():
    '''
    # this creates fields for the image classes
    '''
    def __init__(self):
        self.textField = pm.textFieldButtonGrp(text='image to load',
                width = 430, buttonLabel='<<<', bc=pm.Callback(self.addImage), ed=0 )
        
        self.button = pm.button('button%s' % self.num ,width = 50, label = 'X',
                command = pm.Callback(self.delete))
        
    def delete(self):
        pm.deleteUI(self.textField)
        pm.deleteUI(self.button)
