#Tony Sakson
#SAL_GradingScript_Project_02_v4.py

#How to run:
#	import SAL_GradingScript_Project_02_v4
#	reload (SAL_GradingScript_Project_02_v4)
#
...
def windowGUI():
	window = cmds.window('TSakson SAL Grading Script Project 02 - Film Noir')
...
def updateGradeTotal(*args):
	...
	cmds.intFieldGrp( "gradeIntField", e=1, value1=((lightGradeTotal*.85)+(cflGradeTotal*.15)))
	...
	totalGradeTotal=cmds.intFieldGrp( "totalGradeIntField", e=1, value1=((gradeTotal)-aanqDisplayGradeTotal-proDisplayGradeTotal-lateGradeTotal))
...
# Formats the comments for output	
	...
	sceneFileOutput.write("Comp/Focal Len Grade: "+str(cflGradeTotal)+"\r\n")
	sceneFileOutput.write("\r\n")
...
def compFocalButton1(*args):
	compFocalA="Composition directs the viewers eye to the subject instantaneously. Focal length choice supports the composition."
...
def lightingButton1(*args):
	lightingA="Renders show exemplary understanding of hard light and shadows. The lighting directs the viewers eye to the subject matter instantaneously. The lighting has a strong style based on cinema reference."
	lightingB="Renders show an acceptable understanding of hard lighting.  The lighting directs the viewer's eye to the subject after a few seconds of viewing the image. The lighting has a style based on a cinema reference."
	lightingC="Renders show a basic understanding of hard lighting. The lighting directs the views eye to the subject after a 5 seconds of viewing the image. The lighting has a vague style based on a cinema reference."
...
def lightingButton2(*args):
	lightingD="Renders show below average understanding of hard lighting.  The lighting does not direct the viewer's eye to the subject. No lighting style based on cinema reference."
	lightingF="Renders show unsatisfactory understanding of hard lighting.  A strong lack of appeal of lighting direction and complete lack of style based on cinema reference."
...
def lightingFunction():
	...
	lightGroup1 = cmds.radioButtonGrp( "lightButGrp1", numberOfRadioButtons=3, label='Lighting (85%)', labelArray3=['A+ -A', 'B+ -B', 'C+ -C'], onCommand=lightingButton1 )
