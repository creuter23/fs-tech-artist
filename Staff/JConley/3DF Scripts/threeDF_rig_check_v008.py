...
joint_naming = ('good', 'bad', 'alright', 'Custom')
joint_rotates = ('rotates not frozen', 'some rotates not frozen', 'good job', 'Custom')
grade_list1 = ('A', 'F')
grade_list2 = ('A', 'B', 'C', 'D', 'F')
...
class Widget_Two():
    ...
    def widget2_button(self, *args):
        ...
        if self.pf == 1:
            cmds.columnLayout()
            cmds.rowColumnLayout(nc=2)
            for each in grade_list1:
                cmds.button(l=each, w=75)
            cmds.setParent(self.widget2_main)
        else:
            cmds.columnLayout()
            cmds.rowColumnLayout(nc=5)
            for each in grade_list2:
                cmds.button(l=each, w=40)
            cmds.setParent(self.widget2_main)
            
      
    def widget2_comments(self, *args):
        cmds.popupMenu(parent=self.mainScroll)
        for each in self.comments:
            if each == 'Custom':
                cmds.menuItem(l=each)
            else:
                cmds.menuItem(l=each, c=adding_text)
                self.comment = each
           
    def adding_text(*args):
        cmds.scrollField(self.mainScroll, e=True, it='adding text')
    
            
    def widget2_create_comment(*args):
        win = cmds.window()
        cmds.columnLayout()
        cmds.text(l='Enter in new comment')
        self.new_comment_gui = cmds.textField()
        add = cmds.button(l='Add', c=widget2_add_comment)
        cmds.showWindow(win)
