...
class Frame_Widget():
    def __init__(self, name, objs, parent, width=200, scrollHeight=180, bg_color=[1,0,0], attrs=[]):
        self.width=width
        self.objs = objs
        self.attrs = attrs
...
def gui():
    ...
    frm1 = Frame_Widget('Joint Rotations - Invalid', check_joint_transforms(cmds.ls(typ='joint'), ['rx', 'ry', 'rz']), main)
    frm2 = Frame_Widget('Joint Translations - Invalid', check_joint_transforms(cmds.ls(typ='joint'), ['tx', 'ty', 'tz']), main)
...
def check_joint_transforms(objects, transform):
    '''
    ...
    for obj in objects:
        for attr in transform:
            x = cmds.getAttr('%s.%s' % (obj, attr)) # This should be False
            y = cmds.getAttr('%s.%s' % (obj, attr)) # This should be True
            z = cmds.getAttr('%s.%s' % (obj, attr)) # This should be True
            
            if x > math.fabs(.0001) or y > math.fabs(.0001) or z > math.fabs(.0001):
                # This is invalid
                invalid.append(obj)
                break
