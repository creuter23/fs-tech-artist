...
class Exporter(object):
	...
	def __init__(self, startObj):
		...
		for i, node in enumerate(self.nodes):
			self.info[node].idNum = i
			
		for i, node in enumerate(self.nodes):
			if( self.info[node].children ):
				for child in self.info[node].children:
					idNum = self.info[child].idNum
					self.info[node].childrenID.append(idNum)
			
	'''
	def _setChildrenID(self):
		
		for node in nodes:
			for child in node.children:
				# get the childs id through the node id
				info[child]
				node.childrenID.append( )
	'''
	...
	def slide(self, objects):
		...
		if( Node.confirmType(objects) == 1 ): # Transform
...
class Shape(Node):
	...
	def __init__(self, nodeName):
		...
		# Shade nodes won't have children.
		self.children = None
	def getData(self):
	...
	def binaryInfo(self, littleE=0):
		...
		# MaterialType
		# self.nodeName.encode('ascii')
		# size of material Name
		binData.append(struct.pack('%si' %endian, len(self.material)))
		binData.append(struct.pack('%s%ss' %(endian, len(self.material)), self.material.encode('ascii')))
	...
	def asciiInfo(self):
		...
		# line += [ "%s " %str(i) for i in range(len(self.sets)) ]
		for i,set in enumerate(self.sets):
			line += " " + str(i)
		line += "\n"	
		'''
		line += "\nNumUVS: %s UVSets: %s\n" %(len(self.sets), " ".join(self.sets))	
...
class Transform(Node):
	...
	def binaryInfo(self, littleE=0):
		...
		for mat in self.matrix:
			binData.append(struct.pack( '%sf' %endian, mat))

		binData.append(struct.pack( '%si' %endian, self.numChildren ))
		for child in self.childrenID:
			binData.append(struct.pack( '%si' %endian, child))
		return binData
	...
	def asciiInfo(self):
		...
		# line += "Coords: %s\n" %(" ".join(strNums(self.pos)))
		line += "World Matrix: %s\n" %self.matrix
		line += "NumChildren: %s ChildID(s): %s" %(len(self.childrenID), self.childrenID)
		# children
	...
	def _getMatrix(self):
		return cmds.xform( self.nodeName, q=True, worldSpace=True, matrix=True )
