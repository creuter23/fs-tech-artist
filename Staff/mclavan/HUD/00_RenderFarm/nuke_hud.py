...
def run_job():
    '''
    Get from data from xml
    '''
    current_path = os.path.split(__file__)[0]
    xml_path = os.path.join(current_path, 'hud.xml')
    
    root = ET.parse(xml_path)
    hud = root.getroot()
    print hud[0].text, hud[1].text, hud[2].text
    test_data = {'artist': hud[1].text, 'asset': hud[2].text, 'seq_shot': hud[0].text,
                 'production': hud[3].text, 'department': hud[5].text, 'date': hud[4].text}
    
    test_read = {'file': '/Users/mclavan/Documents/Projects/HUD/img/FireHydrant_LightTest.%03d.iff',
                 'first': int(hud[6][0].text), 'last': int(hud[6][1].text)}
    file_name = 'current_test_2.mov'
    test_write = {'file': os.path.join(hud[8].text, file_name)}
    
    # "C:\Program Files\Nuke6.2v1\Nuke6.2.exe" ".py file directory" --nukex -t
